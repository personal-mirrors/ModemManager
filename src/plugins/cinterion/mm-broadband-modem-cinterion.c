                  ...
                  GTask *task)
{
    ...
    g_assert (active_slot >= 1 && active_slot <= ctx->number_slots);

    ctx->active_slot_index = active_slot;
    MMBaseSim* active_sim = g_ptr_array_index (ctx->sim_slots, active_slot -1);
    if (active_sim != NULL)
