...
interface_syncing_step (GTask *task)
{
    ...
    case SYNCING_STEP_REFRESH_CONNECTION:
        ...
        mm_obj_dbg (self, "refreshing connection bearers");
        /*
        ...
        if (!self->priv->ignore_disconnection_reports) {
            if (!MM_BASE_BEARER_GET_CLASS (self)->reload_connection_status)
                ...
                MM_BASE_BEARER_GET_CLASS (self)->reload_connection_status (self, (GAsyncReadyCallback) reload_connection_status_ready, task);
                return;
            }
        }
        ctx->step++;
