...
static gboolean
split_local_ipv4_address_and_subnet (const gchar  *str,
                                gchar       **local_address,
                                guint       *subnet)
{
    ...
    while (1) {
        ...
        if (separator) {
            ...
            if (count == 4) {
                ...
                if (subnet)
                    *subnet = mm_netmask_to_cidr (++separator);
                return TRUE;
            }
            separator++;
            continue;
...
static gboolean
split_local_ipv6_address_and_subnet (const gchar  *str,
                                gchar       **local_address,
                                guint       *subnet)
{
    const gchar *separator;

    /* E.g. "2001:0DB8:0000:CD30:0000:0000:0000:0000/60"
     * into:
     *    local address: "2001:0DB8:0000:CD30:0000:0000:0000:0000",
     *    subnet: "FFFF:FFFF:FFFF:FFF0:0000:0000:0000:0000"
     */
    g_assert (str);
    g_assert (local_address);
    ...
    if (separator) {
        if (local_address)
            *local_address = g_strndup (str, (separator - str));
        if (subnet)
            *subnet = (guint)g_strtod (++separator,NULL);
        return TRUE;
    }
                                  ...
                                  GError      **error)
{
    ...
    if (local_address_and_subnet && !is_ipv6 && !split_local_ipv4_address_and_subnet (local_address_and_subnet, &local_address, out_subnet_num)) {
        inner_error = g_error_new (MM_CORE_ERROR, MM_CORE_ERROR_FAILED, "Error parsing local address and subnet");
        goto out;
    }
    else if (local_address_and_subnet && is_ipv6 && !split_local_ipv6_address_and_subnet (local_address_and_subnet, &local_address, out_subnet_num)) {
        inner_error = g_error_new (MM_CORE_ERROR, MM_CORE_ERROR_FAILED, "Error parsing local address and subnet");
        goto out;
    }
